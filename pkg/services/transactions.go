...
// GetTransactionsByReceiptId returns all transactions belonging to a receipt
func (s *TransactionService) GetTransactionsByReceiptId(c core.Context, uid int64, receiptId int64) ([]*models.Transaction, error) {
	if uid <= 0 {
		return nil, errs.ErrUserIdInvalid
	}
	...
	err := s.UserDataDB(transaction.Uid).DoTransaction(c, func(sess *xorm.Session) error {
		...
		if transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT {
			if oldTransaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT || oldTransaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_IN {
				...
				transactionUuids := s.GenerateUuids(uuid.UUID_TYPE_TRANSACTION, 1)

				if len(transactionUuids) < 1 {
					return errs.ErrSystemIsBusy
				}

				transaction.RelatedId = transactionUuids[0]
			}
		}
		...
		if transaction.Quantity != oldTransaction.Quantity {
			...
			if transaction.UnitPrice > 0 {
				transaction.Amount = (transaction.Quantity*transaction.UnitPrice + 500) / 1000
		...
		if transaction.Type != oldTransaction.Type {
			updateCols = append(updateCols, "type", "related_id")
		...
		if transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT || transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_IN {
			...
			updateCols = append(updateCols, "related_account_id", "related_account_amount")
		}

		// Если меняется сумма и есть quantity, пересчитать цену
		if transaction.Amount != oldTransaction.Amount && transaction.Quantity > 0 {
			transaction.UnitPrice = (transaction.Amount*1000 + transaction.Quantity/2) / transaction.Quantity
		...
		if transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT || transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_IN {
			...
			oldWasTransfer := oldTransaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT || oldTransaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_IN
			...
			if oldWasTransfer {
				...
				database := s.UserDataDB(transaction.Uid)
				insertRelatedTranSavePointName := "insert_related_tran"
				err = database.SetSavePoint(sess, insertRelatedTranSavePointName)

				if err != nil {
					log.Errorf(c, "[transactions.ModifyTransaction] failed to set save point \"%s\", because %s", insertRelatedTranSavePointName, err.Error())
					return err
				}
				...
				if err != nil || createdRows < 1 {
					if err != nil {
						log.Warnf(c, "[transactions.ModifyTransaction] cannot create related transaction, because %s, regenerate transaction time value", err.Error())
					} else {
						log.Warnf(c, "[transactions.ModifyTransaction] cannot create related transaction, regenerate transaction time value")
					}

					err = database.RollbackToSavePoint(sess, insertRelatedTranSavePointName)

					if err != nil {
						log.Errorf(c, "[transactions.ModifyTransaction] failed to rollback to save point \"%s\", because %s", insertRelatedTranSavePointName, err.Error())
						return err
					}

					sameSecondLatestTransaction := &models.Transaction{}
					minTransactionTime := utils.GetMinTransactionTimeFromUnixTime(utils.GetUnixTimeFromTransactionTime(relatedTransaction.TransactionTime))
					maxTransactionTime := utils.GetMaxTransactionTimeFromUnixTime(utils.GetUnixTimeFromTransactionTime(relatedTransaction.TransactionTime))

					has, err := sess.Where("uid=? AND transaction_time>=? AND transaction_time<=?", relatedTransaction.Uid, minTransactionTime, maxTransactionTime).OrderBy("transaction_time desc").Limit(1).Get(sameSecondLatestTransaction)
					...
					if err != nil {
						log.Errorf(c, "[transactions.ModifyTransaction] failed to create related transaction again, because %s", err.Error())
						return err
					} else if createdRows < 1 {
						log.Errorf(c, "[transactions.ModifyTransaction] failed to create related transaction again")
						return errs.ErrDatabaseOperationFailed
					}
				}
			}
		...
		case models.TRANSACTION_DB_TYPE_INCOME:
			...
			if oldAccountNewAmount != oldTransaction.Amount || transaction.Type != oldTransaction.Type {
				oldSourceAccount.UpdatedUnixTime = time.Now().Unix()

				var balanceExpr string

				if transaction.Type == oldTransaction.Type {
					balanceExpr = fmt.Sprintf("balance-(%d)+(%d)", oldTransaction.Amount, oldAccountNewAmount)
				} else if oldAccountNewAmount != 0 {
					balanceExpr = fmt.Sprintf("balance-(%d)-(%d)", oldTransaction.Amount, transaction.Amount)
				} else {
					balanceExpr = fmt.Sprintf("balance-(%d)", oldTransaction.Amount)
				}

				updatedRows, err := sess.ID(oldSourceAccount.AccountId).SetExpr("balance", balanceExpr).Cols("updated_unix_time").Where("uid=? AND deleted=?", oldSourceAccount.Uid, false).Update(oldSourceAccount)
			...
			if newAccountNewAmount != 0 {
				...
				if transaction.Type == models.TRANSACTION_DB_TYPE_INCOME {
					balanceExpr = fmt.Sprintf("balance+(%d)", newAccountNewAmount)
				} else {
					balanceExpr = fmt.Sprintf("balance-(%d)", newAccountNewAmount)
				}

				updatedRows, err := sess.ID(sourceAccount.AccountId).SetExpr("balance", balanceExpr).Cols("updated_unix_time").Where("uid=? AND deleted=?", sourceAccount.Uid, false).Update(sourceAccount)
		...
		case models.TRANSACTION_DB_TYPE_TRANSFER_OUT:
			...
			if oldSourceAccountNewAmount != oldTransaction.Amount || transaction.Type != oldTransaction.Type {
				oldSourceAccount.UpdatedUnixTime = time.Now().Unix()

				var balanceExpr string

				if transaction.Type == oldTransaction.Type {
					balanceExpr = fmt.Sprintf("balance+(%d)-(%d)", oldTransaction.Amount, oldSourceAccountNewAmount)
				} else if oldSourceAccountNewAmount != 0 {
					if transaction.Type == models.TRANSACTION_DB_TYPE_INCOME {
						balanceExpr = fmt.Sprintf("balance+(%d)+(%d)", oldTransaction.Amount, transaction.Amount)
					} else {
						balanceExpr = fmt.Sprintf("balance+(%d)-(%d)", oldTransaction.Amount, transaction.Amount)
			...
			if newSourceAccountNewAmount != 0 {
				...
				if transaction.Type == models.TRANSACTION_DB_TYPE_INCOME {
					balanceExpr = fmt.Sprintf("balance+(%d)", newSourceAccountNewAmount)
				} else {
					balanceExpr = fmt.Sprintf("balance-(%d)", newSourceAccountNewAmount)
				}
		...
		// Handle destination account when changing from non-transfer to transfer
		if transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT &&
			oldTransaction.Type != models.TRANSACTION_DB_TYPE_TRANSFER_OUT &&
			oldTransaction.Type != models.TRANSACTION_DB_TYPE_TRANSFER_IN &&
			oldTransaction.Type != models.TRANSACTION_DB_TYPE_MODIFY_BALANCE {
			destinationAccount.UpdatedUnixTime = time.Now().Unix()
			updatedRows, err := sess.ID(destinationAccount.AccountId).SetExpr("balance", fmt.Sprintf("balance+(%d)", transaction.RelatedAccountAmount)).Cols("updated_unix_time").Where("uid=? AND deleted=?", destinationAccount.Uid, false).Update(destinationAccount)
		...
		// Unconfirmed -> Confirmed/Verified: apply balance
		// Confirmed/Verified -> Unconfirmed: reverse balance
		// Confirmed <-> Verified: no change
		if transaction.Status != oldTransaction.Status {
			needsApply := oldTransaction.Status == models.TRANSACTION_STATUS_UNCONFIRMED && transaction.Status != models.TRANSACTION_STATUS_UNCONFIRMED
			needsReverse := oldTransaction.Status != models.TRANSACTION_STATUS_UNCONFIRMED && transaction.Status == models.TRANSACTION_STATUS_UNCONFIRMED
			...
			if needsApply || needsReverse {
				...
				switch transaction.Type {
				case models.TRANSACTION_DB_TYPE_MODIFY_BALANCE:
					balanceMod = transaction.RelatedAccountAmount
				case models.TRANSACTION_DB_TYPE_INCOME:
					balanceMod = transaction.Amount
				case models.TRANSACTION_DB_TYPE_EXPENSE:
					balanceMod = -transaction.Amount
				case models.TRANSACTION_DB_TYPE_TRANSFER_OUT:
					balanceMod = -transaction.Amount
				...
				if err != nil {
					log.Errorf(c, "[transactions.ModifyTransaction] failed to update account balance for status change, because %s", err.Error())
					return err
				} else if updatedRows < 1 {
					log.Errorf(c, "[transactions.ModifyTransaction] failed to update account balance for status change")
					return errs.ErrDatabaseOperationFailed
				}
				...
				if transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT && destinationAccount != nil {
					...
					var destBalanceMod int64 = transaction.RelatedAccountAmount
					...
					updatedRows, err := sess.ID(destinationAccount.AccountId).SetExpr("balance", destBalanceExpr).Cols("updated_unix_time").Where("uid=? AND deleted=?", destinationAccount.Uid, false).Update(destinationAccount)

					if err != nil {
						log.Errorf(c, "[transactions.ModifyTransaction] failed to update destination account balance for status change, because %s", err.Error())
						return err
					} else if updatedRows < 1 {
						log.Errorf(c, "[transactions.ModifyTransaction] failed to update destination account balance for status change")
						return errs.ErrDatabaseOperationFailed
					}
				}
			}
		...
		// Sync receipt if transaction belongs to one
		if transaction.Status != oldTransaction.Status && oldTransaction.ReceiptId > 0 {
			err = s.syncReceiptTransactionsStatus(c, sess, transaction.Uid, oldTransaction.ReceiptId, transaction.Status, transaction.TransactionId, now)

			if err != nil {
				log.Errorf(c, "[transactions.ModifyTransaction] failed to sync receipt transactions for receipt \"id:%d\", because %s", oldTransaction.ReceiptId, err.Error())
				return err
			}
		}
...
func (s *TransactionService) DeleteTransaction(c core.Context, uid int64, transactionId int64) error {
	...
	return s.UserDataDB(uid).DoTransaction(c, func(sess *xorm.Session) error {
		...
		// Update account table
		if oldTransaction.Status != models.TRANSACTION_STATUS_UNCONFIRMED {
			switch oldTransaction.Type {
			case models.TRANSACTION_DB_TYPE_MODIFY_BALANCE:
...
// UpdateTransactionStatus updates the status of a transaction and adjusts balance accordingly
func (s *TransactionService) UpdateTransactionStatus(c core.Context, uid int64, transactionId int64, newStatus models.TransactionStatus) error {
	if uid <= 0 {
		return errs.ErrUserIdInvalid
	}
	...
	return s.UserDataDB(uid).DoTransaction(c, func(sess *xorm.Session) error {
		...
		updatedRows, err := sess.ID(transactionId).Cols(updateCols...).Where("uid=? AND deleted=?", uid, false).Update(updateModel)
		if err != nil {
			return err
		} else if updatedRows < 1 {
			return errs.ErrTransactionNotFound
		...
		if transaction.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT {
			...
			has, err = sess.ID(transaction.RelatedId).Where("uid=? AND deleted=?", uid, false).Get(relatedTransaction)
			if err != nil {
				return err
			} else if has {
				...
				_, err = sess.ID(transaction.RelatedId).Cols(updateCols...).Where("uid=? AND deleted=?", uid, false).Update(relatedUpdateModel)
				if err != nil {
					return err
				}
			}
		...
		if transaction.ReceiptId > 0 {
			err = s.syncReceiptTransactionsStatus(c, sess, uid, transaction.ReceiptId, newStatus, transaction.TransactionId, now)

			if err != nil {
				log.Errorf(c, "[transactions.UpdateTransactionStatus] failed to sync receipt transactions for receipt \"id:%d\", because %s", transaction.ReceiptId, err.Error())
				return err
			}
		}
...
// syncReceiptTransactionsStatus syncs the status change to all other transactions in the same receipt and updates the receipt itself
func (s *TransactionService) syncReceiptTransactionsStatus(c core.Context, sess *xorm.Session, uid int64, receiptId int64, newStatus models.TransactionStatus, excludeTransactionId int64, now int64) error {
	var receiptTransactions []*models.Transaction
	err := sess.Where("uid=? AND deleted=? AND receipt_id=? AND transaction_id!=?", uid, false, receiptId, excludeTransactionId).Find(&receiptTransactions)

	if err != nil {
		return err
	}

	for _, rt := range receiptTransactions {
		oldStatus := rt.Status
		...
		rt.Status = newStatus
		rt.UpdatedUnixTime = now

		_, err = sess.ID(rt.TransactionId).Cols("status", "updated_unix_time").Where("uid=? AND deleted=?", uid, false).Update(rt)

		if err != nil {
			return err
		}

		// Adjust balance for this receipt transaction
		needsApply := oldStatus == models.TRANSACTION_STATUS_UNCONFIRMED && newStatus != models.TRANSACTION_STATUS_UNCONFIRMED
		needsReverse := oldStatus != models.TRANSACTION_STATUS_UNCONFIRMED && newStatus == models.TRANSACTION_STATUS_UNCONFIRMED

		if needsApply || needsReverse {
			sourceAccount, destinationAccount, err := s.getAccountModels(sess, rt)
			...
			switch rt.Type {
			case models.TRANSACTION_DB_TYPE_MODIFY_BALANCE:
				balanceMod = rt.RelatedAccountAmount
			case models.TRANSACTION_DB_TYPE_INCOME:
				balanceMod = rt.Amount
			case models.TRANSACTION_DB_TYPE_EXPENSE:
				balanceMod = -rt.Amount
			case models.TRANSACTION_DB_TYPE_TRANSFER_OUT:
				balanceMod = -rt.Amount
			...
			// Handle destination account for transfers
			if rt.Type == models.TRANSACTION_DB_TYPE_TRANSFER_OUT && destinationAccount != nil {
				destinationAccount.UpdatedUnixTime = now
				var destBalanceMod int64 = rt.RelatedAccountAmount
	...
	_, err = sess.ID(receiptId).Cols("status", "updated_unix_time").Where("uid=? AND deleted=?", uid, false).Update(receiptUpdateModel)

	return err
}

// BatchUpdateTransactionsStatus updates the status of multiple transactions and adjusts balance accordingly
func (s *TransactionService) BatchUpdateTransactionsStatus(c core.Context, uid int64, transactionIds []int64, newStatus models.TransactionStatus) error {
	if uid <= 0 {
		return errs.ErrUserIdInvalid
	}
	...
	for i := 0; i < len(transactionIds); i++ {
		err := s.UpdateTransactionStatus(c, uid, transactionIds[i], newStatus)
		if err != nil {
			return err
		}
	}
...
func (s *TransactionService) GetAccountsTotalIncomeAndExpense(c core.Context, uid int64, startUnixTime int64, endUnixTime int64, excludeAccountIds []int64, excludeCategoryIds []int64, clientTimezone *time.Location, useTransactionTimezone bool) (map[int64]int64, map[int64]int64, error) {
	...
	condition := "uid=? AND deleted=? AND status!=? AND (type=? OR type=?)"
	conditionParams := make([]any, 0, 5+len(excludeAccountIds)+len(excludeCategoryIds))
	conditionParams = append(conditionParams, uid)
	conditionParams = append(conditionParams, false)
	conditionParams = append(conditionParams, models.TRANSACTION_STATUS_UNCONFIRMED)
...
func (s *TransactionService) GetAccountsAndCategoriesTotalInflowAndOutflow(c core.Context, uid int64, startUnixTime int64, endUnixTime int64, tagFilters []*models.TransactionTagFilter, noTags bool, keyword string, clientTimezone *time.Location, useTransactionTimezone bool) ([]*models.Transaction, error) {
	...
	condition := "uid=? AND deleted=? AND status!=? AND (type=? OR type=? OR type=? OR type=?)"
...
func (s *TransactionService) GetAccountsAndCategoriesMonthlyInflowAndOutflow(c core.Context, uid int64, startYear int32, startMonth int32, endYear int32, endMonth int32, tagFilters []*models.TransactionTagFilter, noTags bool, keyword string, clientTimezone *time.Location, useTransactionTimezone bool) (map[int32][]*models.Transaction, error) {
	...
	for groupKey, transaction := range transactionsMonthlyAmountsMap {
		...
		if !exists {
			monthlyAmounts = make([]*models.Transaction, 0)
		}
...
func (s *TransactionService) doCreateTransaction(c core.Context, database *datastore.Database, sess *xorm.Session, transaction *models.Transaction, transactionTagIndexes []*models.TransactionTagIndex, tagIds []int64, pictureIds []int64, pictureUpdateModel *models.TransactionPictureInfo) error {
	...
	// Update account table
	if transaction.Status != models.TRANSACTION_STATUS_UNCONFIRMED {
		switch transaction.Type {
		case models.TRANSACTION_DB_TYPE_MODIFY_BALANCE:
...
func (s *TransactionService) getOldAccountModels(sess *xorm.Session, transaction *models.Transaction, oldTransaction *models.Transaction, sourceAccount *models.Account, destinationAccount *models.Account) (oldSourceAccount *models.Account, oldDestinationAccount *models.Account, err error) {
	...
	} else if oldTransaction.RelatedAccountId > 0 {
