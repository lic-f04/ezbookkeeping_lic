          ...
          <v-window-item value="basicInfo">
            <v-form class="mt-2">
              <v-row>
                ...
                <!-- ========== СУММА (с возможностью пересчета) ========== -->
                ...
                <v-col cols="12" md="6" v-if="showQuantityPriceFields">
                  <v-text-field
                    ...
                    :readonly="mode === TransactionEditPageMode.View"
                    :disabled="loading || submitting"
                    :label="tt(sourceQuantityName)"
                    :placeholder="tt(sourceQuantityName)"
...
// ← ДОБАВЛЕНО: Импортируем SetTransactionOptions для совместимости
...
const isTransactionModified = computed<boolean>(() => {
  if (mode.value === TransactionEditPageMode.Add) {
    return transactionsStore.isTransactionDraftModified(
      transaction.value,
      initOptions.value?.amount,
      initOptions.value?.categoryId,
      initOptions.value?.accountId,
      initOptions.value?.tagIds,
      firstVisibleAccountId.value
    );
...
function onAmountChange(): void {
  ...
  if (quantity > 0 && amount > 0) {
    // Цена = Сумма / Количество (округляем до 2 знаков после запятой)
    const calculatedPrice = Math.round((amount / quantity)) / 100;
    transaction.value.unitPrice = calculatedPrice;
  }
}
// ========== КОНЕЦ МЕТОДОВ ПЕРЕСЧЕТА ==========
...
function cancel(): void {
  ...
  if (settingsStore.appSettings.autoSaveTransactionDraft === 'confirmation') {
    if (transactionsStore.isTransactionDraftModified(
      transaction.value,
      initOptions.value?.amount,
      initOptions.value?.categoryId,
      initOptions.value?.accountId,
