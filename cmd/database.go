...
func updateAllDatabaseTablesStructure(c *core.CliContext) error {
	...
	err = datastore.Container.UserDataStore.SyncStructs(new(models.Receipt))

	if err != nil {
		return err
	}

	log.BootInfof(c, "[database.updateAllDatabaseTablesStructure] receipt table maintained successfully")
